...
public class ProductDealsService {
	...
	public ProductDeals getProductDeals(Long id) {
		return productDealsRepo.findById(id).orElse(null);
	}

	public Page<ProductDeals> getProductDeals(Map<String, Long> map, String sku, Pageable pageable) {
		Specification<ProductDeals> spec = new Specification<ProductDeals>() {

			@Override
			public Predicate toPredicate(Root<ProductDeals> root, CriteriaQuery<?> query,
